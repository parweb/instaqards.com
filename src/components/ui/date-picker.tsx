...
const triggerStyles = tv({
  base: [
    // base
    'peer flex cursor-pointer appearance-none items-center gap-x-2 truncate rounded-md border px-3 py-2 shadow-xs outline-hidden transition-all sm:text-sm',
    // background color
    'bg-white dark:bg-gray-950',
    // border color
    'border-gray-300 dark:border-gray-800',
    ...
    return (
      <PopoverPrimitives.Trigger asChild>
        ...
        >
          ...
          <span className="hidden min-w-1/3 flex-1 overflow-hidden text-left text-ellipsis whitespace-nowrap text-gray-900 sm:block dark:text-gray-50">
            {children ? (
              children
            ) : placeholder ? (
...
const PresetContainer = <TPreset extends Preset, TValue>({
  ...
  return (
    <ul className="flex items-start gap-x-2 sm:flex-col">
      {presets.map(preset => {
        return (
          <li key={preset.label} className="sm:w-full sm:py-px">
            <button
              type="button"
              title={preset.label}
              className={cn(
                // base
                'relative w-full overflow-hidden rounded border px-2.5 py-1.5 text-left text-base text-ellipsis whitespace-nowrap shadow-xs outline-hidden transition-all sm:border-none sm:py-2 sm:text-sm sm:shadow-none',
                // text color
...
const SingleDatePicker = ({
  ...
  const onDateChange = (date: Date | undefined) => {
    ...
    if (showTimePicker) {
      if (newDate && !time) {
        setTime(new Time(0, 0) as unknown as TimeValue);
      }
      if (newDate && time) {
  ...
  const onRangeChange = (range: DateRange | undefined) => {
    ...
    if (showTimePicker) {
      if (newRange?.from && !startTime) {
        setStartTime(new Time(0, 0) as unknown as TimeValue);
      }

      if (newRange?.to && !endTime) {
        setEndTime(new Time(0, 0) as unknown as TimeValue);
      }
