...
export const MenuVariation40: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden overflow-hidden">
      <div className="relative bg-gradient-to-t from-amber-700 via-orange-600 to-red-700">
        {/* Effets spéciaux */}
        ...
        <div className="relative px-4 py-4 bg-black/30 backdrop-blur-sm">
          <div className="flex items-center justify-around">
            {tabs.map((tab, index) => (
              ...
              >
                {activeTab === index && (
                  <div className="absolute -inset-2 bg-gradient-to-r from-amber-300/40 via-white/40 to-amber-300/40 rounded-2xl animate-pulse blur-sm" />
                )}
                ...
                <div
                  className={cn(
                    ...
                    activeTab === index
                      ? 'bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 text-white shadow-2xl shadow-amber-500/50 border-white'
                      : 'bg-amber-800 text-amber-200 border-amber-600 hover:bg-amber-700'
                  )}
                >
                  {tab.icon}
                </div>
