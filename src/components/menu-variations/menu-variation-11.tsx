    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden overflow-hidden">
      <div className="relative bg-gradient-to-t from-red-900 via-orange-700 to-yellow-600">
        ...
        {lavaDrops.map(drop => (
          <div
            key={drop.id}
            className="absolute bg-gradient-to-b from-yellow-400 via-orange-500 to-red-600 rounded-full opacity-80 animate-bounce"
            style={{
              left: `${drop.x}%`,
              top: '10%',
              width: `${drop.size}px`,
        ...
        <div className="absolute bottom-0 left-0 w-full h-full pointer-events-none">
          {Array.from({ length: 5 }, (_, i) => (
            <div
              key={i}
              className="absolute bottom-0 bg-gradient-to-t from-red-600 via-orange-500 to-yellow-400 opacity-60 animate-pulse"
              style={{
                left: `${20 + i * 15}%`,
                width: '8px',
          ...
          <div className="flex items-center justify-around">
            {tabs.map((tab, index) => (
              ...
              >
                {/* Aura volcanique */}
                {activeTab === index && (
                  <div className="absolute -inset-2 bg-gradient-to-r from-red-500/40 via-orange-500/40 to-yellow-500/40 rounded-2xl animate-pulse blur-sm" />
                )}
                ...
                <div
                  className={cn(
                    ...
                    activeTab === index
                      ? 'bg-gradient-to-r from-red-500 via-orange-500 to-yellow-500 text-white shadow-2xl shadow-orange-500/50 border-yellow-300'
                ...
                <span
                  className={cn(
                    ...
                    activeTab === index
                      ? 'text-yellow-100 animate-pulse drop-shadow-[0_0_8px_rgba(255,165,0,0.8)]'
                      : 'text-orange-200'
                  )}
                >
                  {tab.name}
                ...
                {activeTab === index && (
                  <>
                    ...
                    <div
                      className="absolute top-1 right-0 w-1 h-1 bg-orange-400 rounded-full animate-ping opacity-75"
                      style={{ animationDelay: '0.3s' }}
                    />
                    <div
                      className="absolute bottom-0 left-1 w-1.5 h-1.5 bg-red-400 rounded-full animate-ping opacity-75"
        ...
        <div className="absolute bottom-0 left-0 w-full h-12 bg-gradient-to-t from-red-700/60 to-transparent blur-md" />
      </div>
    </nav>
  );
};
